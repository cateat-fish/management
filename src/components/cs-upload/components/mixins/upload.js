...
export default {
  data() {
    return {
      token: { file_size: 50000, file_ext: 'txt,doc,pdf,docx,xlsx,xls', image_ext: 'png,jpg' },
      parentId: [],
      parentData: [],
      parentProps: {
        value: 'storage_id',
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      ...
      const fielMaxSize = util.stringToByte(String(this.token['file_size']))
    ...
    handleSuccess(response, file, fileList) {
      if (response.code === 200 && response.result) {
        // if (response.result[0]['status'] !== 200) {
        //   this.handleError(response.data[0]['message'], file, fileList)
        //   return
        // }
        this.$emit('upload', fileList, response.result)
    ...
    handleChange(fileList) {
      if (this.autoUpload) {
        if (
          Object.keys(fileList).every(
            item => fileList[item].status === 'success'
          )
        ) {
          this.loading = false
        }
    ...
    getDirectory() {
      ...
      // getStorageDirectorySelect().then(res => {
